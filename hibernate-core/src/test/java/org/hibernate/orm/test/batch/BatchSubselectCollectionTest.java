...
@DomainModel(
		annotatedClasses = {
				BatchSubselectCollectionTest.EntityA.class,
				BatchSubselectCollectionTest.EntityB.class,
				BatchSubselectCollectionTest.EntityC.class,
				BatchSubselectCollectionTest.EntityD.class,
				BatchSubselectCollectionTest.EntityE.class
		}
)
@SessionFactory
@ServiceRegistry(
...
public class BatchSubselectCollectionTest {
		...
		scope.inTransaction(
				session -> {
					List<EntityD> entitiesD = session.createQuery( "select d from EntityD d", EntityD.class )
							.getResultList();
					assertThat( entitiesD ).hasSize( 1 );
