 ...
 * based on the inferred result type. Spanner PostgreSQL needs this,
 * because by default it assumes a {@code text[]}, which is not compatible with {@code varchar[]} or {@code character varying[]}.
 */
public class SpannerPostgreSQLArrayConstructorFunction extends ArrayConstructorFunction {

	public SpannerPostgreSQLArrayConstructorFunction(boolean list) {
		super( list, true );
	}
	...
	public void render(
		...
		if ( returnType instanceof BasicPluralType<?, ?> pluralType ) {
			if ( needsArrayCasting( pluralType.getElementType() ) ) {
				...
				if ( arrayTypeName.toLowerCase().contains( "character varying[]" ) || arrayTypeName.toLowerCase().contains( "character varying array" ) ) {
					arrayTypeName = "text[]";
				}
