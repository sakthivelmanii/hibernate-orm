...
public class SpannerPostgreSQLJsonInsertFunction extends AbstractJsonInsertFunction {
	...
	public void render(
		...
		// Logic:
		// jsonb_insert(json, path, value, true)
		// Spanner PG supports jsonb_insert and it handles "insert if missing" for
		// object keys automatically.
		...
		List<JsonPathHelper.JsonPathElement> jsonPathElements = JsonPathHelper
				.parseJsonPathElements(translator.getLiteralValue(jsonPath));
	...
	private static boolean isJsonType(Expression expression) {
		return expression.getExpressionType() != null
				&& expression.getExpressionType().getSingleJdbcMapping().getJdbcType().isJson();
	}
}
