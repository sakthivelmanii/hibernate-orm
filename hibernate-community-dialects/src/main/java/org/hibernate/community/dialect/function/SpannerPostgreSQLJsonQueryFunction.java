...
/**
 * Spanner PostgreSQL json_query function.
 */
public class SpannerPostgreSQLJsonQueryFunction extends JsonQueryFunction {

	public SpannerPostgreSQLJsonQueryFunction(TypeConfiguration typeConfiguration) {
	...
	private void appendJsonQuery(
		...
		if (wrapMode == JsonQueryWrapMode.WITH_WRAPPER) {
			...
			sqlAppender.appendSql(
					"(select case when count(*) over () > 1 then jsonb_agg(t.v) else percentile_disc(0) within group (order by t.v) end from jsonb_path_query(");
		} else {
		...
		// ALWAYS cast to jsonpath for Spanner
		// Spanner PG doesn't support casting string literal to jsonpath?
		// Try removing cast and rely on implicit conversion or string passing
		// sqlAppender.appendSql( "cast(" );
		jsonPath.accept(walker);
		// sqlAppender.appendSql( " as jsonpath)" );

		if (passingClause != null) {
			// passing clause arguments also need to be cast to jsonb if they are not
			// already?
			// The error message for passing was: function jsonb_path_query(jsonb, unknown,
			// jsonb) does not exist
			// The third argument is the vars map. jsonb_build_object returns jsonb, so that
			// should be fine.
			// But wait, the second argument (path) was 'unknown'.
